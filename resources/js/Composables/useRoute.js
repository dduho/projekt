...
function buildFallbackUrl(name, params = {}) {
    ...
    const routes = {
        ...
        'projects.comments.store': (p) => `/projects/${p.project || p.id || p}/comments`,
        'projects.analyze-risks': (p) => `/projects/${p.id || p}/analyze-risks`,
        'comments.update': (p) => `/comments/${p.comment || p.id || p}`,
        'comments.destroy': (p) => `/comments/${p.comment || p.id || p}`,
        'import.index': '/import',
        'import.store': '/import',
